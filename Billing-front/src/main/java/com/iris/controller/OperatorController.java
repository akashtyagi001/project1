...
/* @ResponseBody
 ...
 List<ProjectAllocation> getAllProjectAllocation(){
		...
		*/
		...
		@ResponseBody
		@RequestMapping(value= {"/submitEmployeeAttendance"},method=RequestMethod.POST)
		public ResponseEntity<?> addconfig(@RequestBody Operator dataObj){
			...
			try {
				...
				
				{
					...
					dataObj.setEmpObj(employeeService.getEmployeeById(dataObj.getEmployeeId()));
					dataObj.setProjObj(projectService.getProjectById(dataObj.getProjectId()));
					boolean saved = operatorService.setAttandance(dataObj);
